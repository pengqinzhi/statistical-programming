...
```{r warning=FALSE}
library(corrplot)
corrplot(cor(df), method="ellipse", tl.pos="n")
```
...
ggplot(data = data.frame(pve), aes(x=1:20, y=cumsum(pve))) +
  geom_line()+
  geom_point() +
...
```
```
To explain 90% of the overall variance, I would like to retain 5 PCs.
```
...
```{r}
ggplot(data = df, aes(x=pca.out$x[,1], y=pca.out$x[,2], color=response)) +
  geom_point() +
  xlab("PC 1") +
  ylab("PC 2")
```
...
```{r}
# for PC2 and PC3
ggplot(data = df, aes(x=pca.out$x[,2], y=pca.out$x[,3], color=response)) +
  geom_point() +
  xlab("PC 2") +
  ylab("PC 3")

# for PC3 and PC4
ggplot(data = df, aes(x=pca.out$x[,3], y=pca.out$x[,4], color=response)) +
  geom_point() +
  xlab("PC 3") +
  ylab("PC 4")
```
...
```{r}
ggplot(data = df, aes(x=response, y=pca.out$x[,1])) +
  geom_boxplot() +
  ylab("PC 1") 
```
...
glm.pred <- ifelse(glm.prob>0.5, "M","B")
(t <- table(glm.pred,response))
cat("The misclassification rate for 1 PC is ", round((t[1,2]+t[2,1])/sum(t),3),"\n\n")
...
glm.pred <- ifelse(glm.prob>0.5, "M","B")
(t <- table(glm.pred,response))
cat("The misclassification rate for 5 PC is ", round((t[1,2]+t[2,1])/sum(t),3),"\n")
```

```
Note: Multicollinearity affects the coefficients and p-values, but it does not influence the predictions, precision of the predictions, and the goodness-of-fit statistics.
